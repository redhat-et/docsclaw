...
// NewWebFetchTool creates a new web fetch tool with the given configuration.
func NewWebFetchTool(cfg WebFetchConfig) tools.Tool {
...
func (t *webFetchTool) Execute(ctx context.Context, args map[string]any) *tools.ToolResult {
	...
	if err != nil {
		return tools.Errorf("invalid URL: %s", err)
	}

	// Only allow HTTP and HTTPS schemes
	scheme := strings.ToLower(parsed.Scheme)
	if scheme != "http" && scheme != "https" {
		return tools.Errorf("unsupported URL scheme: %s", parsed.Scheme)
	}

	if len(t.config.AllowedHosts) > 0 {
		if !t.isHostAllowed(parsed.Hostname()) {
			return tools.Errorf("Host not allowed: %s", parsed.Hostname())
		}
	}
	...
	if err != nil {
		return tools.Errorf("failed to create request: %s", err)
	}
	...
	if err != nil {
		return tools.Errorf("failed to read response: %s", err)
	}

	if resp.StatusCode != http.StatusOK {
		return tools.Errorf("HTTP %d: %s", resp.StatusCode, string(body))
	}

	return tools.OK(string(body))
}
